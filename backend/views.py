...
def search_api():
    ...
    if url_hash or url:
        if url_hash is not None:
            ...
            # TODO: do this enforcing when we move to the new UI
        ...
        json['objects'] = list(set([s.pageview.url for s in scripts]))
...
def align_webpage_data():
    ...
    def exp_filter(resources, condition):
        """ WARNING: *condition* must never be user-controlled """
        new_resources = {}
        for k in resources.keys():
            placed = False
            for new_k in new_resources.keys():
                if eval(condition) and not date_collision_present(resources[k], new_resources[new_k]):
                    eval(rekey)
    ...
    for pv in webpage.pageviews:
        resources[pv.date] = [{'url': s.url, 
                               'hash': s.hash, 
                               'parent_url': s.pageview.url, 
                               'date': s.pageview.date} for s in pv.scripts]
    ...
    resources = exp_filter(resources,   
                           "resources[k][0]['url']).path.split('/')[-1] == urlparse(new_resources[new_k][0]['url']).path.split('/')[-1]", 
